...
def latest_version():
    ...
    files = [int(f.split('.')[0].split('_v')[1]) for f in files if f.endswith('.hpp')]
...
def external_EXEC(command: str, without_output: bool = False):
    res = Popen(command, shell=True, stdout=PIPE, stderr=PIPE, encoding='utf-8')
    ret_code = res.wait()
...
def compile(version: str = latest, gpu: bool = False):
    ...
    with QproDefaultConsole.status("编译中"):
        code, content = external_EXEC(f"hipcc -Ofast -std=c++11 -I include -I kernel -I {roc_include} -L {roc_lib} {'-D gpu' if gpu else ''} -D VERSION='<{job_name}_v{version}.hpp>' -lomp -fopenmp -lrocsparse main.cpp -o {executable}", True)
    if code:
        QproDefaultConsole.print(QproErrorString, content.replace('errors', '[bold red]errors[/bold red]').replace('warnings', '[bold yellow]warnings[/bold yellow]').replace('error', '[bold red]error[/bold red]').replace('warning', '[bold yellow]warning[/bold yellow]'))
    else:
    ...
    def show_log():
        if os.path.exists(f'log/{job_id}.loop'):
            ...
            with open(f'log/{job_id}.loop', 'r') as f:
                ct = f.read().strip()
            QproDefaultConsole.print(Panel(ct, title='[bold magenta]当前输出[/bold magenta]', width=QproDefaultConsole.width))

    with QproDefaultConsole.status("查询状态中"):
        code, content = external_EXEC(f"squeue --name={job_name}", without_output=True)
    if code:
        return
    ...
    if not items:
        ...
        with open(f'log/{job_id}.loop', 'r') as f:
            ...
            if ct[-2].startswith('Congratulation'):
                QproDefaultConsole.print(QproInfoString, f'计算[bold green]通过[/bold green]，版本 "{version}" 性能：{gflops} GFlop/s')
                if version not in record:
                    record[version] = gflops
                    os.system(f'cp kernel/{job_name}_{version}.hpp template/{job_name}_{version}.hpp')
                    QproDefaultConsole.print(QproInfoString, f'版本 "{version}" 的最佳实现已保存')
                    dump_record()
                ...
                QproDefaultConsole.print(QproErrorString, f'计算[bold red]错误[/bold red]，版本 "{version}" 性能：{gflops} GFlop/s')
        return
    for item in items:
        table.add_row(*item)
...
def show():
    ...
    for item in sorted(list(record.keys()), key=lambda x: int(x[1:])):
        table.add_row(item, str(record[item]), '[bold green]√[/bold green]' if record[item] == max(record.values()) else '')
    QproDefaultConsole.print(table, justify='center')
